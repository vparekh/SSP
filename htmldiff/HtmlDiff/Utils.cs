...
namespace HtmlDiff
{
    public static class Utils
    {
        ...
        public static string WrapText(string text, string tagName, string cssClass, string id, int StartIndex, int StopIndex)
        {
            //@"<ins onclick='remove(this, ""mod"")' class='mod'>";
            return string.Format("<{0} id={2} class='{1}' startIndex={3} stopIndex={4} onclick=showposition(this)>{5}</{0}>", tagName, cssClass, id, StartIndex, StopIndex, text);
            //return string.Format(@"<{0} class='{1}' onclick='remove(this,""{1}"")'>{2}</{0}>", tagName, cssClass, text);
        }
